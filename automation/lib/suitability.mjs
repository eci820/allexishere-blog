...
// v2.7 가점: 💰금융·🏠부동산 +1.5(CPC 최상위), 💪건강·🔬생활원리형 +1.25, 🔬과학지식형 +0.75. 가십·인물 실검 감점 유지.
...
export function scoreKeyword(c, stat) {
  ...
  if (c.source === 'calendar') {
    ...
    tierTag = '건강';
  } else if (c.source === 'finance') {
    pts += 1.5; // 💰 금융·재테크·세금 — CPC 최상위
    tierTag = '금융';
  } else if (c.source === 'realestate') {
    pts += 1.5; // 🏠 대출·부동산 — CPC 최상위
    tierTag = '부동산';
