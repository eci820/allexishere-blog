...
⚠️ 제목 규칙 — 아래 둘을 **모두** 지키세요.
 ① 중복 발행 방지: 시설명과 '주차'를 **띄어쓴 별개 단어로** 넣으세요.
    '주차장'·'주차요금'처럼 붙여 쓰면 '주차'로 인식되지 않아 같은 시설 글이 또 생성됩니다.
 ② pain point 나열: [시설명] + [주차] + [pain point 2~3개] + [가이드/정리]
    축: 요금 · 위치·입구 · 혼잡·만차 · 할인 · 근처 대체 중 실제 검색 수요가 있는 2~3개.
    ❌ "잠실야구장 주차 총정리"                       ← 사실상 검색어 하나만 잡음
    ✅ "잠실야구장 주차 요금·입구 위치·혼잡 대비 가이드"  ← '주차' 독립 단어 + 세 의도
    독자는 "총정리"를 검색하지 않습니다. 구체적 니즈를 각각 검색합니다.
 🔴 제목에 넣은 항목은 위 h2 로 실제 내용이 있어야 합니다. 확인 못 한 건 제목에도 넣지 마세요.`;
...
⚠️ 제목 규칙: [주제/대상] + [독자 pain point 2~3개 구체 나열] + [가이드/정리/안내]
   ❌ "○○ 총정리"  ← 독자는 "총정리"를 검색하지 않습니다. 구체적 니즈를 각각 검색합니다.
   ✅ 방법·시간/타이밍·주의사항·대상별 차이 중 실제 검색 수요가 있는 2~3개를 나열.
      예) "유산균 총정리" → "유산균 보관법·복용 시간·냉장 여부 안내"
   🔴 제목에 나열한 항목은 h2 로 실제 내용이 있어야 합니다(순서도 맞춤).
      확인 못 한 항목은 제목에 넣지 마세요 — 제목에 있고 본문에 없으면 낚시입니다.`;
